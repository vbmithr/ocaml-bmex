...
let transfersw =
  let open Kx in
  t10 timestamps syms syms syms syms (v guid)
    (list (s char))
    (list (s char))
    floats floats
...
let kx_of_transfers transfers =
  ...
  List.iteri transfers
    ~f:(fun i
      ...
      refids.(i) <- transactID;
      times.(i) <-
        Option.(value ~default:Kx.np (first_some transactTime timestamp));
      syms.(i) <- Option.value_exn currency;
      txids.(i) <- Option.value_exn tx;
      addrs.(i) <- Option.value_exn address;
      amounts.(i) <-
        Option.value_map amount ~default:Kx.nf ~f:(fun amount ->
            amount // 100_000_000);
      fees.(i) <-
        Option.value_map fee ~default:Kx.nf ~f:(fun amount ->
            amount // 100_000_000));
  let open Kx in
  Kx_async.create
    ...
    ( "upd",
      "transfers",
      (times, syms, xchs, kinds, statuses, refids, txids, addrs, amounts, fees)
    )
...
let retrieveTransfers w =
  let rec inner start =
    Bmex_rest.walletHistory ~testnet:true ~key ~secret ~start ~count:10000 ()
    >>= fun txs ->
    match List.length txs with
    | 0 -> Deferred.unit
    | len ->
        Log_async.app (fun m -> m "Found %d txs" len) >>= fun () ->
        Pipe.write w (kx_of_transfers txs) >>= fun () -> inner (start + len)
  in
  inner 0

let indexw = Kx.(t3 (v sym) (v timestamp) (v float))
...
let retrieveIndex w symbol =
  let rec inner startTime =
    ...
    | h :: _ as idxVals ->
        ...
        let nextTs =
          Caml.Option.get @@ Ptime.add_span h.timestamp (Ptime.Span.of_int_s 1)
        in
        inner nextTs
  in
  let startTime =
...
let () =
  ...
  Command.group ~summary:"bmex army knife"
    [
      ( "compta",
        Command.async ~summary:"Accounting experiment"
          (let open Command.Let_syntax in
          [%map_open
            let () = Logs_async_reporter.set_level_via_param [] in
            fun () ->
              Kx_async.with_connection url (fun { w; _ } -> retrieveTransfers w)])
      );
      ( "index",
        Command.async ~summary:"Store historical indices in DB"
          (let open Command.Let_syntax in
          [%map_open
            ...
            fun () ->
              let url =
                Uri.make ~userinfo:"tickerplant:pass" ~scheme:"kdb"
                  ~host:"localhost" ~port:6000 ()
              in
              Kx_async.with_connection url (fun { w; _ } ->
                  Deferred.List.iter symbols ~f:(fun symbol ->
                      retrieveIndex w symbol))]) );
    ]
  |> Command.run
