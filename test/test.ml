...
let wrap_request ?(timeout = Time.Span.of_int_sec 5) ?(speed = `Quick) n service
    =
  test_case ~timeout n speed (fun () ->
      Fastrest.request service |> Deferred.Or_error.ignore_m
      |> Deferred.Or_error.ok_exn)

let wrap_request_light ?(timeout = Time.Span.of_int_sec 5) ?(speed = `Quick) n f
    =
  test_case ~timeout n speed (fun () -> f () |> Deferred.ignore_m)
...
let orderCycle () =
  ...
  let price = 6000. in
  let o =
    createOrder ~price ~symbol:"XBTUSD" ~orderQty:1 ~ordType:Limit clOrdID
  in
...
let rest =
  [
    ...
    test_case "executionHistory" `Quick (fun () ->
        raise_on_error
          (executionHistory ~testnet:true ~key ~secret ~symbol:"XBTUSD"
             ~ts:date));
    ...
    test_case "cancelAllAfter" `Quick (fun () ->
        raise_on_error (fun () ->
            cancelAllAfter ~testnet:true ~key ~secret
              (Time_ns.Span.of_int_sec 10)));
  ]

let main () = run "bmex" [ ("rest", rest) ]
